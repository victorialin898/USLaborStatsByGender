  ...
  dplyr::mutate(
    occupation = factor(
      ...
      levels = MedianWeeklyEarningsByOccupationAggregate |>
        dplyr::arrange(desc(men_median_weekly_earnings)) |>
...
p1 <- ggplot(top_15_women, aes(x = occupation, y = wage_gap, fill = wage_gap)) +
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 0.5), axis.text.y = element_text(size=5), plot.title = element_text(size = 10))  # Rotate x-axis labels for better readability
